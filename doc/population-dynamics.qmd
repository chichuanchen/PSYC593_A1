...
```
 ...
 World CBR went down from period 1950-1955 to 2005_2010, and same thing for both
...
mean(kenya_data$asfr)
mean(sweden_data$asfr)
t.test(kenya_data$asfr, sweden_data$asfr)
```
The data showed that, on average, women in Kenya had a higher age-specific fertility rate than women in Sweden.
...
(changes_totals <- total_birth_world[2, -1] / total_birth_world[1, -1])
(change_women <- total_birth_world$total_women[total_birth_world$period == "2005-2010"] -
  total_birth_world$total_women[total_birth_world$period == "1950-1955"])
(change_births <- total_birth_world$total_births[total_birth_world$period == "2005-2010"] -
  total_birth_world$total_births[total_birth_world$period == "1950-1955"])
```

In general, the number of women and the number of births both increased from the 1950s to the 2000s.
...
```
Sweden has a sligtly lower CDR than the world average, while Kenya
has a slightly higher CDR than the world average.
...
sweden_data$asdr
```
Overall, Sweden seems to have lower age-specific death rates than both the world's and Kenya.
...
kenya_data %>%
  mutate(temp_cdr = asdr * sweden_data$popP) %>%
  group_by(period) %>%
  summarise(cdrresweden = sum(temp_cdr))

kenya_data %>%
  mutate(temp_cdr = asdr * kenya_data$popP) %>%
  group_by(period) %>%
  summarise(cdrrekenya = sum(temp_cdr))
```
